...
- **ATUALIZADO — widget descontinuado:** a integração não usa mais o Pluggy Connect (widget/SDK); o app coleta credenciais bancárias em telas nativas e o backend as repassa diretamente à API REST do Pluggy (`POST /items`, `POST /items/{id}/mfa`) via `pluggy-client.service.ts`. O backend **processa** essas credenciais em memória (nunca as persiste em banco/log) para repassá-las ao Pluggy — ver nota de compliance em `00-DECISIONS.md §2` e `specs/security/lgpd.md §1` (pendente validação jurídica formal desta mudança antes de produção). O backend consome a mesma API para buscar contas, cartões e transações após o `itemId` ser criado.
...
- **Sincronização:** webhook do Pluggy (`item/updated`) + fallback de sync manual (mutation `syncOpenFinanceConnection`, usada em pull-to-refresh) + job diário de reconciliação (fora do escopo deste módulo GraphQL — job agendado separado, **ainda não implementado** neste MVP). **Observado em teste em dispositivo real:** o webhook nem sempre chega/é processado a tempo (suspeita: cold start da function serverless — que faz `NestFactory.createApplicationContext(AppModule)` completo — estourando o limite de 5s que o Pluggy exige para considerar a entrega bem-sucedida; webhook registrado e endpoint funcionalmente corretos, confirmado chamando o endpoint manualmente). Por isso o app mobile (`useOpenFinanceConnections`) chama `syncOpenFinanceConnection` automaticamente a cada ~8s enquanto houver conexão em `UPDATING` (até 30 tentativas), como rede de segurança client-side — reduz a dependência do webhook chegar a tempo sem esperar pela implementação do job diário de reconciliação.
- **Consentimento:** formulário nativo do próprio app Vidinha (não mais o widget Pluggy Connect) — ver nota de compliance acima.
