...
## 12. Lançamentos manuais, "Carteira"/"Carteira digital" e foto de perfil

- **Contas/cartões/transações manuais:** `Account`/`Card` continuam sendo entidades **separadas** no modelo de dados (decisão da seção 1 mantida — granularidade de compartilhamento "por conta e por cartão" exige isso, mesmo o modelo da Pluggy tratando conta e cartão de forma unificada). O que muda é que ambas passam a suportar CRUD manual completo (`createAccount`/`updateAccount`/`archiveAccount`, `createCard`/`updateCard`/`archiveCard`, `createTransaction`/`updateTransaction`/`deleteTransaction`) para usuários que não querem ou não podem usar Open Finance para uma conta/cartão específico. Uma conta/cartão manual (`isManual: true`, `connectionId: null`) é somente editável pelo próprio dono; uma conta/cartão sincronizado via Open Finance (`isManual: false`) é somente leitura, exceto compartilhamento. `Transaction.source` (`OPEN_FINANCE | MANUAL | RECURRING_EXPENSE`) documenta a origem de cada lançamento.
- **"Carteira" (dinheiro em espécie) e "Carteira digital" (cripto):** modeladas como **novos valores do enum `AccountType` já existente** (`CASH`, `CRYPTO`) em vez de novas entidades — reaproveita 100% da infraestrutura de saldo/compartilhamento/transação de `Account` já existente. Sempre `isManual: true`, sem `connectionId` (não existe integração Open Finance para dinheiro em espécie ou cripto).
- **Importação de extrato (planilha/CSV):** o parse acontece **inteiramente no app** (client-side); o servidor nunca recebe o arquivo bruto, apenas linhas já validadas via chamadas sequenciais de `createTransaction`. Isso elimina a superfície de segurança de upload de arquivo (malware, tamanho, tipo MIME) que uma rota de upload de planilha teria — decisão tomada explicitamente para reduzir a superfície de ataque em vez de adicionar validação adicional a ela.
- **Foto de perfil (avatar):** bucket **privado** `avatars` no Supabase Storage (não bucket público). O backend nunca expõe uma URL fixa: toda leitura de `User.avatarUrl` no GraphQL computa uma **signed URL de leitura** de curta duração on-demand a partir do path salvo (`resolveAvatarUrl`), visível a qualquer usuário que já possa ver aquele `User` hoje — isto é, também aos demais membros da família, não só ao dono da foto. O upload usa uma **signed URL de escrita** de curta duração (`createAvatarUploadUrl`), nunca a service_role key do lado do cliente. Ver `specs/security/file-uploads.md` para limites (5MB, jpeg/png/webp) e `specs/security/lgpd.md §1` para o mapeamento de dado pessoal correspondente.
