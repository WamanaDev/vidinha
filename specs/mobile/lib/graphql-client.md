# `apps/mobile/src/lib/graphqlClient.ts`

Cliente GraphQL fino (`graphql-request`) com injeção de JWT a cada requisição e uma única tentativa de refresh de sessão em caso de `UNAUTHENTICATED`. Ver estratégia completa de estado/JWT em [`../00-overview.md`](../00-overview.md#33-injeção-de-jwt-e-tratamento-de-expiração) e armazenamento seguro em [`secure-storage.md`](./secure-storage.md).
